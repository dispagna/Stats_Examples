...
Finally, I'll check the posterior predictive distribution. Using the posterior samples, I can plot the expected value of the posterior predictive distribution.
...
It's also possible to have _stan_ generate the posterior predictive distribution by adding a _generated quantities_ section to the model similar to what I did for the prior predictive distribution. 
...
mdl_code_ppd <- '
  ...
  generated quantities{
    ...
    real mpg_ppd[N] = normal_rng(a + b * disp, sigma);
  }
'
...
Finally, I'll check the posterior predictive distribution.  This time I plot both the 89% credible interval for the expected vale of _mpg_ and the 89% credible interval for the posterior predictive distribution of _mpg_.   
...
# Method 1: Plot individual lines
# post_pred <- apply(draws, 1, function(x) {
#   x["a"] + B %*% x[grepl("w", names(x))]
# }) %>%
#   as.data.frame() %>%
...
# Method 2a: Calculate 89% interval for expected value of posterior predictive
# Epost_pred <- apply(draws, 1, function(x) {
#   x["a"] + B %*% x[grepl("w", names(x))]
# }) %>%
...
# Method 2b: Calculate 89% interval for expected value of posterior predictive from Y_hat
...
# Method 4: Use posterior predictive from generated quantities
...
One challenge with splines is choosing the number of knots.  For the previous model, I tried several values for _num_knots_ until settling on 4.  However, there is a case study for splines with stan that uses a novel prior which addresses this issue.  The details are (here)[https://mc-stan.org/users/documentation/case-studies/splines_in_stan.html].
...
The plot above shows that even with a large number of knots (in this case 20), the model with the smoothing prior prevents over-fitting when compared to the model without the smoothing prior.
